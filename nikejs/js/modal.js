export default class Modal {
  ...
  onDocumentClick(event) {
    ...
    if (targetButton) {
      ...
      return this.open(target, speed ? parseInt(speed) : 300);
    }
  ...
  open(targetWindow, speed = 300) {
    ...
    // Если передан targetWindow, используем его, иначе берем текущее modalWindow
    if (targetWindow) {
    ...
    if (!this.modalWindow) {
      console.warn('Modal window not found');
      return;
    }
